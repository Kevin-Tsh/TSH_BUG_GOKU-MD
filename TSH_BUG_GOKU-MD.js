...
senddocu(buffer)
        ...
        switch (command) {
            ...
            case 'restart':
                if (!isCreator) return replygcxeon(mess.owner)
                replygcxeon('*Goku est en cours de redémarrage...*')
                exec('pm2 restart all')
                break
            case 'autoread':
                if (!isCreator) return replygcxeon(mess.owner)
                if (args.length < 1) return replygcxeon(`Exemple ${prefix + command} on/off`)
                if (q === 'on') {
                    autoread = true
                    replygcxeon(`*Autoread changé avec succès sur ${q}*`)
                } else if (q === 'off') {
                    autoread = false
                    replygcxeon(`*Autoread changé avec succès sur ${q}*`)
                }
    ...
    case 'autostatusview':{
            ...
            case 'getcase':
                if (!isCreator) return replygcxeon(mess.owner)
                const getCase = (cases) => {
                    return "case" + `'${cases}'` + fs.readFileSync("TSH_BUG_GOKU-MD.js").toString().split('case \'' + cases + '\'')[1].split("break")[0] + "break"
                }
...
┌────────────────═•⟢
  ...
  │𝙊𝙬𝙣𝙚𝙧 : *${ownername}*
  │𝙋𝙧𝙚𝙛𝙞𝙭 :  *[ ${prefix} ]*
  │𝙈𝙤𝙙𝙚 : *${XeonBotInc.public ? 'Public' : `Self`}*
  │𝙏𝙚𝙢𝙥𝙨 : ${xeonytimewisher}
  │𝘿𝙖𝙩𝙚 : *Regarde le calendrier !*
  │𝙑𝙚𝙧𝙨𝙞𝙤𝙣 : *2.1*
  │𝙋𝙡𝙪𝙜𝙞𝙣𝙨 : *250*
  │𝙍𝙖𝙢 : *128 GB*
  │𝘼𝙡𝙞𝙫𝙚 : *${runtime(process.uptime())}*
  │🌟𝗳𝗮𝗺𝗶𝗹𝗹𝗲 𝗚𝗿𝗶𝗺𝗺🌟
  └──────────────⫽
└────────────────═•⟢
━━━━━━━━━━━━━━━━━━━━━━━━━━
